...
chem.Struct.Atom = function(params) {
	...
	util.ifDef(this, params, 'reactivity', def('reactivity'))

	this.atomList = !Object.isUndefined(params.atomList) && params.atomList != null ? new chem.Struct.AtomList(
		params.atomList) : null;
...
chem.Struct.prototype.partitionLoop = function(loop) {
	...
	search: while (continueFlag) {
		var
		atomToHalfBond = {}; // map from every atom in the loop to the index of the first half-bond starting from that atom in the uniqHb array
...
chem.Struct.prototype.findLoops = function() {
	...
	this.halfBonds.each(function(i, hb) {
		if (hb.loop == -1) {
			for (j = i, c = 0, loop = []; c <= this.halfBonds.count(); j = this.halfBonds.get(j).next, ++
				c) {
				if (c > 0 && j == i) { // loop found
					var subloops = this.partitionLoop(loop);
					util.each(subloops, function(loop) {
						if (this.loopIsInner(loop) && !this.loopHasSelfIntersections(
							loop)) { // loop is internal
							...
							this.loops.set(loopId, new chem.Loop(loop, this, this.loopIsConvex(
								loop)));
						} else {
							loopId = -2;
						}
