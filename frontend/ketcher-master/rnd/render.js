...
rnd.Render.prototype.update = function (force)
{
	...
	this.setReactivity(null);// Meanwhile redraw the Reactive part.
};
...
// Update in 7.9
rnd.Render.prototype.setReactivity = function (selection)
{
	rnd.logMethod("setReact");
	for (var map in rnd.ReStruct.maps) {
        if (!rnd.ReStruct.maps[map].isSelectable())
            continue;
		var set = selection ? (selection[map] ? util.identityMap(selection[map]) : {}) : null;
		this.ctab.atoms.each(function(id, item){
            var selected = set ? set[id] === id : ui.ctab.atoms._map._obj[id].reactivity;
			this.ctab.showItemReactivity(id, item, selected);
		}, this);
	}
};