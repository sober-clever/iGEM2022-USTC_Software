...
// Updated in 7.9
rnd.ReStruct.prototype.showItemReactivity = function (id, item, visible)
{
	var exists = (item.reactivityPlate != null) && !item.reactivityPlate.removed;
    // rbalabanov: here is temporary fix for "drag issue" on iPad
    //BEGIN
    // exists = exists && (!('hiddenPaths' in rnd.ReStruct.prototype) || rnd.ReStruct.prototype.hiddenPaths.indexOf(item.selectionPlate) < 0);
    //END
	if (visible) {
		if (!exists) {
			...
			var styles = render.styles;
			var paper = render.paper;
			item.reactivityPlate = item.makeReactivityPlate(this, paper, styles);
			this.addReObjectPath('reactivity-plate', item.visel, item.reactivityPlate);
		}
		if (item.selectionPlate) item.reactivityPlate.show(); // TODO [RB] review
	} else {
		if (exists)
			if (item.selectionPlate) item.reactivityPlate.hide(); // TODO [RB] review
	}
};