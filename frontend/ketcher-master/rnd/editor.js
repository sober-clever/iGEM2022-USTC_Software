...
rnd.Editor.prototype.toolFor = function(tool) {
    if (tool == 'selector_lasso') {
        return new rnd.Editor.LassoTool(this, 0);//选择
    } else if (tool == 'selector_square') {
        return new rnd.Editor.LassoTool(this, 1);//选择 方形
    } else if (tool == 'selector_fragment') {
        return new rnd.Editor.LassoTool(this, 1, true);//选择 六边型
    ...
    // new add #####
     else if (tool.startsWith('groups_')){
        return new rnd.Editor.TemplateTool(this, rnd.groups[parseInt(tool.split('_')[1])]);
    }
		else if (tool.startsWith('custom_')){
		    return new rnd.Editor.TemplateTool(this, rnd.custom[parseInt(tool.split('_')[1])]);
		}
    return null;
};
...
rnd.Editor.LassoTool.prototype = new rnd.Editor.EditorTool();//原型改为rnd.Editor.EditorTool生成的对象
